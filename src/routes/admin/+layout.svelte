...
<Drawer variant="modal" bind:open={menuOpen}>
	...
	<Content>
		<List>
			<Item
				href="{base}/admin"
				activated={$page.url.pathname === base + '/admin'}
			...
			{#if data.authSession}
				<Item
					...
					activated={$page.url.pathname === base + '/admin/courses'}
					on:click={() => (menuOpen = !menuOpen)}
				>
							...
							<AccordionContent>
								<List>
									{#each data.allSessions[i] as session}
										<Item
											href="{base}/admin/courses/{course.id}/{session.id}"
											activated={$page.url.pathname ===
												`${base}/admin/courses/${course.id}/${session.id}`}
											on:click={() => (menuOpen = !menuOpen)}
										>
