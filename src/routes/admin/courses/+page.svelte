<script lang="ts">
	import type { BaseStudyProgram, Course, StudyProgram } from '$lib/api';
	...
	let sort: keyof Course = 'id',
		...
		newCourseStudyProgram: StudyProgram | undefined = form?.data?.studyProgramId
			? data.studyPrograms.find((p) => p.id === form?.data?.studyProgramId)
			: undefined,
...
>
	...
	<Body>
		{#each data.courses as course, idx (course.id)}
			...
			<Row on:SMUIDataTableRow:click={() => console.log(course.title)}>
				<Cell numeric>{course.id}</Cell>
				<Cell>{course.title}</Cell>
				<Cell>{course.subtitle || '—'}</Cell>
				<Cell title={studyProgram?.subtitle ?? ''}
					>{studyProgram?.title ?? course.studyProgramId}</Cell
				>
				<Cell>
					{#if course.moodleUrl}
						<a
							class="icon-button moodle"
							href={course.moodleUrl}
							title="View course on eCampus"
							target="_blank"
						>
							<Fa icon={ciMoodle} />
						</a>
				...
				<Cell>{data.allSessions[idx].length}</Cell>
				<Cell>
...
<div id="course-create-open">
	<Button
		...
		on:click={() => (courseCreateOpen = true)}
	>
...
>
	<Header>
		<Title id="course-create-title">Add new course</Title>
	</Header>
			...
			<div class="form-group">
				<Autocomplete
					...
					on:SMUIAutocomplete:noMatchesAction={() => {
						newStudyProgramTitle = newCourseStudyProgramText;
	...
	<Actions>
		<Button defaultAction type="reset" form="new-course" action="close">
