import type {PageServerLoad, PageServerLoadEvent, PageServerData, Actions} from "./$types";
...
export const load: PageServerLoad = async ({params, parent, fetch}: PageServerLoadEvent): Promise<PageServerData> => {
    const courseId = Number(params.course);
    const {authSession, courses, sessions, studyPrograms} = await parent();
    ...
    return {
        ...
        courseSessions: sessions[courseIdx],
        studyProgram: studyPrograms.find((sp: StudyProgram) => sp.id === courses[courseIdx].studyProgramId)
    };
}
...
export const actions = {
    update: async ({request, fetch, params}) => {
        ...
        if(!title || !studyProgramId){
            const missing = [!title && 'title', !studyProgramId && 'studyProgramId'].filter(Boolean) as string[];
            return fail(400, {
                data: payload,
                missing,
            });
