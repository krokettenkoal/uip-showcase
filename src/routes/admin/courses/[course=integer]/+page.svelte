<script lang="ts">
    ...
    let sort: keyof Session = 'id',
        ...
        studyProgramText: string = data.studyProgram.title,
    ...
    const originalCourse: Course = {...data.course},
        originalStudyProgram: StudyProgram = {...data.studyProgram};
    ...
    function hasChanges(c?: Course, sp?: StudyProgram, ...params: any[]): boolean {
        return c?.title !== originalCourse.title
...
<BackButton href="{base}/admin/courses">
    Courses
</BackButton>

<h1 class="mdc-typography--headline3">
    {data.course.title}
    <span class="mdc-theme--text-secondary-on-background mdc-typography--headline5">
        | Edit
    </span>
</h1>
        ...
        <form id="edit-course" method="POST" action="?/update">
            <div class="fields">
                ...
                <div class="form-group">
                    <Autocomplete
                            ...
                            on:SMUIAutocomplete:noMatchesAction={() => {newStudyProgramTitle = studyProgramText; studyProgramCreateOpen = true;}}
                            label="Study program"
                            textfield$required
                    >
            ...
            <div class="actions">
                <Button
                        ...
                        form="edit-course"
                        accesskey="s"
                        disabled={!data.course.title || !data.studyProgram || !hasChanges(data.course, data.studyProgram)}
                >
...
<section id="sessions">
    ...
    >
        ...
        <Body>
        {#each data.courseSessions as session, idx (session.id)}
            <Row>
                ...
                <Cell>
                    {#if session.image}
                        <a class="icon-button image" href="{base}/img/sessions/{session.image}" title="Open session image" target="_blank">
    ...
    <div id="session-create-open">
        <Button variant="outlined" color="primary" class="btn-shaped-round" on:click={() => sessionCreateOpen = true}>
            <ButtonIcon class="material-icons">add</ButtonIcon>
