<script lang="ts">
	...
	let topAppBar: TopAppBar,
		...
		panelOpen: { [session: string]: boolean } = {};
...
<Drawer variant="modal" bind:open={menuOpen}>
	...
	<Content>
		<List>
			<Item
				href="{base}/"
				activated={$page.url.pathname === base + '/'}
			...
			<Item
				href="{base}/courses"
				activated={$page.url.pathname === base + '/courses'}
				on:click={() => (menuOpen = !menuOpen)}
			>
					...
					<Panel bind:open={panelOpen[course.id]} variant="unelevated">
						<AccordionHeader>
							...
							<IconButton slot="icon" toggle pressed={panelOpen[course.id]}>
								<Badge class="program-badge" position="outset" align="middle-start"
									>{studyProgramName(course)}</Badge
								>
						...
						<AccordionContent>
							<List>
								{#each data.allSessions[i] as session}
									<Item
										href="{base}/courses/{course.id}/{session.id}"
										activated={$page.url.pathname === base + `/courses/${course.id}/${session.id}`}
										on:click={() => (menuOpen = !menuOpen)}
									>
