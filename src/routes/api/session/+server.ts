...
const querySql =
	'SELECT Id as id, CourseId as courseId, Title as title, Subtitle as subtitle, Image as image, Date as date FROM `sessions` WHERE CourseId = ?';
...
export const POST: RequestHandler = async ({ params, locals, request }: RequestEvent) => {
	...
	if (!data.courseId || !data.title) {
		const missing = [data.courseId && 'courseId', data.title && 'title'].filter(
			Boolean
		) as string[];
