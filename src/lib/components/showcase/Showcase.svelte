<script lang="ts" context="module">
	...
	async function loadHighlightLanguage(
		sourceType: SourceType | undefined
	): Promise<LanguageType<string>> {
		if (!sourceType?.language) return xml;

		return (
			(
				await import(
					`../../../../node_modules/svelte-highlight/languages/${sourceType.language}.js`
				)
			).default ?? xml
		);
	}
	...
	async function loadHighlightStyle(): Promise<string> {
		...
		return (
			await import(
				`../../../../node_modules/svelte-highlight/styles/github${themeSuffix}.css?inline`
			)
		).default;
	}
</script>
...
<script lang="ts">
	...
	let active: Source = sources[0],
		types: SourceType[] = [],
		activeType: SourceType | undefined,
		highlightStyle: string = `<style>${defaultStyle}</style>`;
	...
	async function updateHighlightStyle() {
		try {
			const s = await loadHighlightStyle();
			highlightStyle = `<style>${s}</style>`;
		} catch (e) {
