<script lang="ts">
	import Card, { Content, Media, Actions } from '@smui/card';
	...
	onMount(async () => {
		...
		types = await Promise.all(
			filterUnique(sources.map((s) => s.typeId)).map((t) => getSourceType(t))
		);
	});
...
<Card {...$$restProps} style="--i:{i}">
	<Media
		...
		aspectRatio="16x9"
		style="background-image:url({base}/img/examples/{example.image || 'placeholder.jpg'})"
	/>
